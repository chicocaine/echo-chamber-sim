...
def run_simulation_streaming(
    ...
    """Generator version of run_simulation for WebSocket streaming (Phase 7 Step 7.1).

    Yields one dict per tick with current metrics, agent opinions, and edge changes.
    Returns the final result dict when the simulation completes.
    """
    merged_config: dict[str, Any] = {**DEFAULT_CONFIG, **(config or {})}
    _validate_config(merged_config)
    ...
    campaign_expiry: dict[int, int] = {}
    previous_shared_content: dict[int, list[Content]] = {}
    previous_edge_set: set[tuple[int, int]] = set(G.edges)
    ...
    predecessor_ids_by_agent = {a.id: get_predecessors(G, a.id) for a in agents if a.id in G}
    neighbors_by_agent = {
        a.id: [G.nodes[nid]["agent"] for nid in predecessor_ids_by_agent[a.id]]
        for a in agents if a.id in G
    }
    influence_weights_by_agent = {
    ...
    for tick in range(total_ticks):
        ...
        current_tick_pool: list[Content] = []
        for agent in active_agents:
            mc = maybe_generate_content(agent, content_id_counter, tick, rng)
            if mc is not None:
                current_tick_pool.append(mc)
                cid = mc.coordinated_campaign_id
                if cid is not None and cid not in campaign_expiry:
                    campaign_expiry[cid] = tick + CAMPAIGN_EXPIRY_DELTA
                content_id_counter += 1

        # Step 2: feed generation (content-based only for streaming perf)
        candidate_pools: dict[int, list[Content]] = {}
        for agent in active_agents:
            pool: list[Content] = list(current_tick_pool)
            for pid in predecessor_ids_by_agent.get(agent.id, []):
                pool.extend(previous_shared_content.get(pid, []))
            candidate_pools[agent.id] = pool

        feeds: dict[int, list[Content]] = {}
        for agent in active_agents:
            pool = candidate_pools[agent.id]
            if not pool:
                feeds[agent.id] = []
                continue
            ideo = np.fromiter((c.ideological_score for c in pool), dtype=np.float64, count=len(pool))
            vir = np.fromiter((c.virality for c in pool), dtype=np.float64, count=len(pool))
            mis = np.fromiter((c.misinfo_score for c in pool), dtype=np.float64, count=len(pool))
            feeds[agent.id] = generate_feed_vectorized(
                agent=agent, candidate_pool=pool,
                content_ideo_array=ideo, content_virality_array=vir,
                content_misinfo_array=mis,
                k_exp=k_exp, alpha=alpha, beta_pop=beta_pop,
                lambda_penalty=lambda_penalty, diversity_ratio=diversity_ratio,
            )

        # Steps 3/4/5: serial processing for streaming
        step_results = [
            _process_agent_tick(
                agent=agent, feed=feeds.get(agent.id, []),
                neighbors=neighbors_by_agent.get(agent.id, []),
                weights=influence_weights_by_agent.get(agent.id, {}),
                alpha=alpha, emotional_decay=emotional_decay,
                arousal_share_weight=arousal_share_weight,
                valence_share_weight=valence_share_weight,
                virality_dampening=virality_dampening,
                random_seed=_agent_tick_seed(base_seed, tick, agent.id),
            )
            for agent in active_agents
        ]

        consumed_items = {a.id: feeds.get(a.id, []) for a in active_agents}
        shared_content: dict[int, list[Content]] = {}
        new_opinions: dict[int, float] = {}
        new_arousals: dict[int, float] = {}
        for aid, new_op, new_ar, shared in step_results:
            shared_content[aid] = shared
            new_opinions[aid] = float(new_op)
            new_arousals[aid] = float(new_ar)
        ...
        for agent in active_agents:
            ...
            for campaign_id, items in misinfo_by_campaign.items():
                ...
                if state == "S":
                    should_infect = False
                    for content in items:
                        agent.exposure_count[campaign_id] = agent.exposure_count.get(campaign_id, 0) + 1
                        n = agent.exposure_count[campaign_id]
                        eff_beta = min(sir_beta * (1.0 + n * reinforcement_factor), 1.0)
                        if py_rng.random() < eff_beta:
                            should_infect = True
                            break
                    if should_infect:
                        agent.sir_states[campaign_id] = "I"
        ...
        edge_list = [[u, v] for u, v in G.edges] if edges_changed else []
        ...
        opin_array = [float(a.opinion) for a in agents]

        yield {
            "tick": tick,
            "metrics": metrics,
            "agent_opinions": opin_array,
